...
```bash
sudo -u tgbot mkdir /opt/telegramBotEmbed/.venv # 为tgbot用户创建虚拟环境目录
sudo -u tgbot /usr/bin/python3 -m venv /opt/telegramBotEmbed/.venv # 创建虚拟环境 (可能需要安装 python3-venv 包: sudo apt install python3-venv)
...
在 `/opt/telegramBotEmbed/config.py` 文件中配置以下内容：
