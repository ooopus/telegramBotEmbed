...
```bash
sudo mkdir -p /opt/telegramBotEmbed
sudo wget -P /opt/telegramBotEmbed https://raw.githubusercontent.com/ooopus/telegramBotEmbed/refs/heads/main/telegramBotEmbed.py
sudo wget -P /opt/telegramBotEmbed https://raw.githubusercontent.com/ooopus/telegramBotEmbed/refs/heads/main/config.example.py
sudo mv /opt/telegramBotEmbed/telegramBotEmbed.py /opt/telegramBotEmbed/main.py
sudo mv /opt/telegramBotEmbed/config.example.py /opt/telegramBotEmbed/config.py
```
...
```bash
sudo -u tgbot mkdir /opt/telegramBotEmbed/.venv # Create a virtual environment directory for the tgbot user
sudo -u tgbot /usr/bin/python3 -m venv /opt/telegramBotEmbed/.venv # Create a virtual environment (may need to install python3-venv package: sudo apt install python3-venv)

# Activate the virtual environment (required before starting the bot)
sudo -u tgbot bash -c "source /opt/telegramBotEmbed/.venv/bin/activate"
...
```bash
sudo -u tgbot /opt/telegramBotEmbed/.venv/bin/pip3 install pyTelegramBotAPI requests numpy
...
In the `/opt/telegramBotEmbed/config.py` file, configure the following:
